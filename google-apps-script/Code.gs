...
// ── Backfill prices for released games with no price set ─────
// Run this manually from the Apps Script editor:
//   select "updateMissingPrices" in the function dropdown → click Run
function updateMissingPrices() {
  ...
  const targets = [];
  for (let i = 1; i < rows.length; i++) {
    const releaseDate = String(rows[i][col('releaseDate')] || '').trim();
    const price       = String(rows[i][col('price')]       || '').trim();
    const steamAppId  = String(rows[i][col('steamAppId')]  || '').trim();

    const isIsoDate   = /^\d{4}-\d{2}-\d{2}$/.test(releaseDate);
    const isReleased  = isIsoDate && releaseDate <= today;

    if (isReleased && !price && steamAppId) {
      targets.push({ rowIndex: i + 1, steamAppId, title: String(rows[i][col('title')] || '') });
    }
  }

  Logger.log('Released games with no price: ' + targets.length);
  if (targets.length === 0) return;

  const priceCol = col('price') + 1; // Sheets columns are 1-based
  let updated = 0;

  targets.forEach(({ rowIndex, steamAppId, title }) => {
    try {
      const res  = UrlFetchApp.fetch(`${STEAM_PROXY}/?appid=${steamAppId}`, { muteHttpExceptions: true });
      const json = JSON.parse(res.getContentText());
      const entry = json[steamAppId];

      if (!entry || !entry.success || !entry.data) {
        Logger.log(`  SKIP ${title} (${steamAppId}): no Steam data`);
        return;
      }
      ...
      if (d.price_overview && d.price_overview.initial != null) {
        price = (d.price_overview.initial / 100).toFixed(2);
      } else if (d.is_free) {
        price = '0.00';
      }

      if (price === null) {
        Logger.log(`  SKIP ${title} (${steamAppId}): no price on Steam`);
        return;
      }

      sheet.getRange(rowIndex, priceCol).setValue(price);
      Logger.log(`  OK   ${title} (${steamAppId}): €${price}`);
      updated++;
      Utilities.sleep(1000); // stay polite to the Steam API
    } catch (err) {
      Logger.log(`  ERR  ${title} (${steamAppId}): ${err.message}`);
    }
  });

  Logger.log(`Done. Updated ${updated} of ${targets.length} games.`);
}
